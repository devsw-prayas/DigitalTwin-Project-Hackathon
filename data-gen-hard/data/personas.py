...
No interface changes from original — same Persona and RiskParams dataclasses,
same load_personas() signature. Added optional fields for new divergent personas.
"""
...
class RiskParams:
    ...
    drift_per_day: float
    event_day: int
    event_magnitude: float          # can be negative (improvement events)
...
if __name__ == "__main__":
    ...
    for name, p in personas.items():
        print(
            f"  {name:20s}  seed={p.seed:4d}  "
            f"cardio={p.cardio.base:.2f}({p.cardio.drift_per_day:+.5f}/d)  "
            f"mental={p.mental.base:.2f}({p.mental.drift_per_day:+.5f}/d)"
            + ("  [DIVERGENT]" if p.cardio.drift_per_day * p.mental.drift_per_day < 0 else "")
        )
